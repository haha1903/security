...
public final class TimeLookupStrategy implements LookupStrategy {
    ...
    private final Field fieldAces = FieldUtils.getField(TimeAcl.class, "aces");
     ...
     */
    public TimeLookupStrategy(DataSource dataSource, AclCache aclCache,
    ...
    private List<AccessControlEntryImpl> readAces(TimeAcl acl) {
        try {
            return (List<AccessControlEntryImpl>) fieldAces.get(acl);
        } catch (IllegalAccessException e) {
            throw new IllegalStateException("Could not obtain TimeAcl.aces field", e);
        }
    }

    private void setAclOnAce(AccessControlEntryImpl ace, TimeAcl acl) {
        try {
            fieldAcl.set(ace, acl);
        } catch (IllegalAccessException e) {
            throw new IllegalStateException("Could not or set TimeAcl on AccessControlEntryImpl fields", e);
        }
    }

    private void setAces(TimeAcl acl, List<AccessControlEntryImpl> aces) {
        try {
            fieldAces.set(acl, aces);
        } catch (IllegalAccessException e) {
            throw new IllegalStateException("Could not set TimeAcl entries", e);
        }
    }

    /**
     * Locates the primary key IDs specified in "findNow", adding TimeAcl instances with StubAclParents to the
     * "acls" Map.
     *
     * @param acls    the TimeAcls (with StubAclParents)
            ...
            if ((currentBatchToLoad.size() == this.batchSize) || ((i + 1) == objects.size())) {
                if (currentBatchToLoad.size() > 0) {
                    ...
                    for (Acl loadedAcl : loadedBatch.values()) {
                        aclCache.putInCache((TimeAcl) loadedAcl);
                    }
    ...
    private Map<ObjectIdentity, Acl> lookupObjectIdentities(final Collection<ObjectIdentity> objectIdentities, List<Sid> sids) {
        ...
        for (Acl inputAcl : acls.values()) {
            Assert.isInstanceOf(TimeAcl.class, inputAcl, "Map should have contained an TimeAcl");
            Assert.isInstanceOf(Long.class, ((TimeAcl) inputAcl).getId(), "Acl.getId() must be Long");

            Acl result = convert(acls, (Long) ((TimeAcl) inputAcl).getId());
    ...
    /**
     * The final phase of converting the <code>Map</code> of <code>TimeAcl</code> instances which contain
     * <code>StubAclParent</code>s into proper, valid <code>TimeAcl</code>s with correct ACL parents.
     *
     * @param inputMap        the unconverted <code>TimeAcl</code>s
     * @param currentIdentity the current<code>Acl</code> that we wish to convert (this may be
     */
    private TimeAcl convert(Map<Serializable, Acl> inputMap, Long currentIdentity) {
        Assert.notEmpty(inputMap, "InputMap required");
        Assert.notNull(currentIdentity, "CurrentIdentity required");

        // Retrieve this Acl from the InputMap
        Acl uncastAcl = inputMap.get(currentIdentity);
        Assert.isInstanceOf(TimeAcl.class, uncastAcl, "The inputMap contained a non-TimeAcl");

        TimeAcl inputAcl = (TimeAcl) uncastAcl;
        ...
        // Now we have the parent (if there is one), create the true TimeAcl
        TimeAcl result = new TimeAcl(inputAcl.getObjectIdentity(), (Long) inputAcl.getId(), aclAuthorizationStrategy,
        ...
        // Create a list in which to store the "aces" for the "result" TimeAcl instance
        List<AccessControlEntryImpl> acesNew = new ArrayList<AccessControlEntryImpl>();

        // Iterate over the "aces" input and replace each nested AccessControlEntryImpl.getAcl() with the new "result" TimeAcl instance
        ...
        // Finally, now that the "aces" have been converted to have the "result" TimeAcl instance, modify the "result" TimeAcl instance
    ...
    private class ProcessResultSet implements ResultSetExtractor<Set<Long>> {
        ...
        /**
         * Accepts the current <code>ResultSet</code> row, and converts it into an <code>TimeAcl</code> that
        ...
        private void convertCurrentResultIntoObject(Map<Serializable, Acl> acls, ResultSet rs) throws SQLException {
            ...
            if (acl == null) {
                // Make an TimeAcl and pop it into the Map
                ...
                acl = new TimeAcl(objectIdentity, id, aclAuthorizationStrategy, grantingStrategy, parentAcl, null,
            ...
            if (rs.getString("ace_sid") != null) {
                ...
                TimeAccessControlEntryImpl timeAccessControlEntry = new TimeAccessControlEntryImpl(aceId, acl, recipient, permission, granting,
                        auditSuccess, auditFailure);
                timeAccessControlEntry.setStart(rs.getTimestamp("start"));
                timeAccessControlEntry.setEnd(rs.getTimestamp("end"));
                AccessControlEntryImpl ace = timeAccessControlEntry;

                //Field acesField = FieldUtils.getField(TimeAcl.class, "aces");
                List<AccessControlEntryImpl> aces = readAces((TimeAcl) acl);
